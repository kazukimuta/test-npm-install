...
class QuickSearchManager {
    ...
    async loadItems(queryModel) {
        ...
        executor.requestMaxTime = this.collectionTimeLimit;
    ...
    indexSearchFormResult(items, doNotSplitResults = false) {
        let result = items;
        ...
        if (!doNotSplitResults && (this.maxReturnedCount < result.length)) {
    ...
    async indexSearch() {
        ...
        const searchCallback = async (key, value, stats) => {
            ...
            const { items, usedCapacityUnits, lastEvaluatedKey } = await this.loadItems(queryModel);
        ...
        if (reservationKey && this.dates.hasReservation() && parallelSearchManager.getLastEvaluatedKeyOfKey(reservationKey)) {
            resultItems = this.indexSearchFormResult(fullResult, true);
            lastEvaluatedKey = parallelSearchManager.getLastEvaluatedKeyOfKey(reservationKey);
        }
        else {
            resultItems = this.indexSearchFormResult(fullResult, false);
            lastEvaluatedKey = (resultItems.length === 0 || resultItems.length === fullResult.length)
                ? undefined
    ...
    async searchAndGetResults() {
        if (this.preloadConfig.full) {
            return await this.fullDataPreload();
        }
        if (this.hasSomethingForFieldSearch()) {
            return await this.indexSearch();
        }
        return await this.searchByResultsMetaAttributes();
    }
}
